...
@section('content')
    <div class="row mb-3">
        <div class="col-5 mx-auto">
            <form action="{{ route('admin.edit_staff.post', $staff) }}" method="post">
                @csrf
                <div class="form-group">
                    <label for="formGroupExampleInput2">Name</label>
                    <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="Full Name"
                           name="nama_staff" value="{{ $staff->nama_staff }}" readonly>
                    @error('nama_staff')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="Email"
                           name="email_staff" value="{{ $staff->email_staff }}" readonly>
                    @error('email_staff')
                    <small id="emailHelp" class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" id="tanggal_masuk" name="tanggal_masuk"
                           value="{{ date('Y-m-d', strtotime($staff->tanggal_masuk)) }}" readonly>
                    @error('tanggal_masuk')
                    <small id="emailHelp" class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" id="tanggal_lahir" name="tanggal_lahir"
                           value="{{ date('Y-m-d', strtotime($staff->tanggal_lahir)) }}" readonly>
                    @error('tanggal_lahir')
                    <small id="emailHelp" class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <select id="inputState" class="form-control" name="id_position">
                        <option selected
                                value="{{ $staff->position->id_position }}">{{ $staff->position->nama_position }}</option>
